...
export default function SongHandler(props) {
  ...
  if (songState === SongState.RECORD) {
    return html`<${VideoRecorder} song=${song} setBlob=${setBlob} setSongState=${setSongState} />`;
  }
  if (songState === SongState.PLAYBACK) {
    return html`<${VideoPlayback} blob=${blob} setSongState=${setSongState} user=${props.user} song=${song}/>` 
  }
